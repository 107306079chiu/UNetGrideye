...
while(1):
	try:
		...
		print('')
		print("Something wrong with the serial connection !")
...
while(end!=True):
	...
	if(ser.in_waiting > 391):
		#print(ser.in_waiting)
		#print("HELLO")

		data = ser.readline().decode()
		temp_values = data.split(',')
		temp_values[64] = temp_values[64][0:5]
		deviceTemp = float(temp_values.pop())

		#to float
		temp_values = np.array(temp_values, dtype=np.float32)
		...
		rgbDisplay = frame[:,240:1680]
		rgbDisplay = cv2.resize(rgbDisplay, (display_size,int(display_size*0.75)), interpolation=cv2.INTER_NEAREST)

		overlay = cv2.addWeighted(rgbDisplay,0.4,grideyeDisplay,0.5,20)

		
		#cv2.namedWindow('rgb',cv2.WINDOW_NORMAL)
		#cv2.resizeWindow('rgb', display_size, display_size)

		cv2.imshow("grideye", grideyeDisplay)
		...
		if(record==True):#record data
			recording = True
			med_activate = False #JUST IN CASE
			ma_activate = False

			rgb_buffer.append(rgbDisplay)
			#temp_buffer.append(temp_values)
			...
			np.save('./grideye_dataset/temp/'+save_id+'.npy',temp_buffer)

			os.mkdir('./grideye_dataset/rgb/'+save_id)
			for i in range(len(rgb_buffer)):
				cv2.imwrite('./grideye_dataset/rgb/'+save_id+'/'+str(i)+'.jpg',rgb_buffer[i])
